...
function Connection( x, y, z, size, parent ){
...
Connection.prototype.link = function(){
	...
	if( links.length === 0 ){
		...
		for( var i = 0; i < links.length; ++i ){
			...
			var pos = links[ i ],
					connection = new Connection( pos.x, pos.y, pos.z, this.size * opts.sizeMultiplier, this );
...
Data.prototype.reset = function(){
	...
	this.setConnection( ends[ Math.random() * ends.length |0 ] );
	this.ended = 2;
}
...
Data.prototype.setConnection = function( connection ){
	...
	if( connection.isRoot )
		...
		this.nextConnection = connection.parent;
...
function anim(){
	...
	if( data.length < connections.length * opts.dataToConnections ){
		var datum = new Data( ends[ Math.random() * ends.length |0 ] );
		data.push( datum );
		all.push( datum );
	}
